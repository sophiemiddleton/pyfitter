...
| File | Description | Role in Analysis |
| :--- | :--- | :--- |
| `fit_module.py` | **Main Fitting Orchestrator** | Central module that takes the defined PDFs from the component modules, combines them into the full model (Signal + Backgrounds), defines the **Negative Log-Likelihood (NLL)** loss function, and executes the `zfit` minimization. |
...
| `output_fitresult.csv` | **Output** | Stores the final fit parameters, errors, and figure of merit (e.g., minimum NLL value). |
